...
- **Release Assets Mirrored to Cloudflare R2 for Web Downloads**: Release automation mirrors every installer to an R2 bucket and generates the manifest the download page consumes — per-file SHA-256 checksums, sizes, and the release's real publish date — with immutable caching for versioned assets, a five-minute TTL on the root manifest, and pruning beyond the five most recent versions. Forks without the R2 credentials skip the sync entirely.
...
- The updater endpoint list ships inside the app binary, so existing installations keep checking GitHub until they have updated once to a build containing this change; from then on the mirror is preferred with GitHub as fallback.
...
- **Code0.ai Invite Link Updated to Agent Register URL**: The Code0.ai sponsor's `apiKeyUrl` moves from the `?source=ccswitch` referral to the new agent-register invite link `https://code0.ai/agent/register/B2XHxGjGmRvqgznY`, updated across all seven app presets and every localized README supported at the time. The bare `code0.ai` API endpoint stays untouched.
...
- **new-api Sponsor Row in READMEs**: The open-source AI infrastructure project `new-api` is appended as the newest entry in the sponsor table across every localized README supported at the time, along with its banner asset.

### Internal

- **Harden Release Supply Chain**: Added a `.github/CODEOWNERS` that, together with branch protection's Code Owners rule, requires owner review before any PR merges to `main` (with `/.github/` and `/src-tauri/` pinned explicitly alongside the global `*` fallback, all to the then-project owner), gated the release job behind a `release` environment in `release.yml` so signing secrets unlock only after manual approval, and removed `.github` from `.gitignore` so the previously untracked `labeler.yml` and `workflows/labeler.yml` are now versioned.
...
- **"Write Common Config" Renamed to "Apply Common Config"**: The original label "Write Common Config" (写入通用配置) was ambiguous about data-flow direction, reading as "write the current config INTO the common config" when the actual behavior is the reverse — the saved common-config snippet is merged INTO this provider's config. The checkbox was renamed across every locale supported at the time, including every hint/guide/notice reference and the `CommonConfigEditor` / `GeminiConfigSections` defaultValue fallbacks. The current repository now retains Chinese and English only. (#4829)
...
- **Documented the `CC_SWITCH_GDK_BACKEND` Escape Hatch in README and User Manual**: Added an FAQ entry for the opt-in `CC_SWITCH_GDK_BACKEND` environment variable (see the corresponding Fixed entry) across every localized README and user-manual troubleshooting page supported at the time, explaining how Wayland + NVIDIA users can switch back to native Wayland when the webview goes click-dead and black-screens on resize, and how tiling-Wayland users can set it to `x11` for the inverse input bug.
- **Overseas Kimi READMEs Point to platform.kimi.ai With a Coding Plan Link**: Updated the Kimi K2.7 Code partner section across the overseas README variants supported at the time so the banner image and both inline CTAs point to `https://platform.kimi.ai?aff=cc-switch` instead of the old `platform.kimi.com` host, keeping the `aff=cc-switch` referral tag intact. Every localized README also gained a line promoting the Kimi For Coding subscription plan linked to `https://www.kimi.com/code/?aff=cc-switch`; note the Chinese README kept its existing `platform.kimi.com` link and only received the Coding Plan addition.
- **GitHub Global Top-100 Milestone Banner in v3.16.4 Release Notes**: Prepended a celebratory callout blockquote to every localized v3.16.4 release note supported at the time, announcing that CC Switch had entered the global top 100 GitHub projects by star count and thanking users, contributors, and stargazers. This was a docs-only addition and did not change product behavior.
...
- **Simplified Homebrew Install Instructions**: The installation guide no longer instructs users to run `brew tap` a third-party cask tap before `brew install --cask cc-switch`; the deprecated tap step was removed from the en/ja/zh user manuals so the cask installs directly. (#4319)
...
- **German README**: Added a German README translation and linked it from the existing README language switchers (#2994).
...
- **CC Switch Brand Surface Unified to the Official Website**: All in-app and README references now point at the official website as the sole official channel; the release notes template also surfaces the official website.
...
- **Release Notes Template**: Surfaces the official website in the release notes template.
- **Brand Surface**: Documented the official website as the sole official channel across READMEs and in-app references.
