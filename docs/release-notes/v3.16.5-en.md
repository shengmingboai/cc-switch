...
The original label "Write Common Config" was ambiguous about data-flow direction (it read like "write the current config into the common config"), whereas the actual behavior is the reverse — it merges the saved common-config snippet into this provider's config. The checkbox was renamed to "Apply Common Config" across every locale supported at the time. The current repository now retains Chinese and English only. ([#4829](https://github.com/farion1231/cc-switch/pull/4829))
