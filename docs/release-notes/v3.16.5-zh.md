...
原标签「写入通用配置」在数据流向上有歧义（读起来像「把当前配置写进通用配置」），而实际行为相反——是把已存的通用配置片段合并进本供应商配置。复选框在当时支持的全部语言中更名为「应用通用配置」。当前仓库现仅保留中文与英语。（[#4829](https://github.com/farion1231/cc-switch/pull/4829)）
