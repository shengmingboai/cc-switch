...
//!
//! Handles reading and writing live configuration files for supported providers.
...
fn restore_live_settings_for_provider_backfill(
    ...
    // `modelCatalog` is a ai-switch–private field whose SSOT is the DB. Live's
...
pub fn read_live_settings(app_type: AppType) -> Result<Value, AppError> {
    match app_type {
        AppType::Codex => {
            ...
            // `modelCatalog` is a ai-switch private field that lives only in
            // the DB SSOT plus the `ai-switch-model-catalog.json` projection
...
pub fn import_default_config(state: &AppState, app_type: AppType) -> Result<bool, AppError> {
    ...
    let settings_config = match app_type {
        ...
        // OpenCode and Pi use additive mode and are handled by early return above
        AppType::OpenCode | AppType::Pi => {
...
/// into the AI Switch database. Each provider found will be added to the
