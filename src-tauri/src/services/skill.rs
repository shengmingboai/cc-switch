...
///
/// 归档字节由第三方完全控制（仓库可经 skill 导入添加，且 branch 可把下载落点
...
impl SkillService {
    ...
    fn extract_repo_archive<R: std::io::Read + std::io::Seek>(
        ...
        // 归档字节完全由第三方控制（仓库可经 skill 导入添加），所以解压必须限量，
        ...
        for i in 0..archive.len() {
            ...
            // （即逃出归档自身根目录）的条目。skill 仓库可由 skill 导入添加，
            // 压缩包内容属第三方可控输入。
...
mod tests {
    ...
    fn validate_repo_ref_accepts_real_world_coordinates() {
        ...
        for branch in [
            ...
            assert!(
                SkillService::validate_repo_ref("example-owner", "example-repo", branch).is_ok(),
    ...
    fn validate_repo_ref_accepts_the_empty_branch_sentinel() {
        ...
        assert!(
            SkillService::validate_repo_ref("example-owner", "example-repo", "").is_ok(),
