...
//! - 实际文件存储在 ~/.ai-switch/skills/，同步到各应用目录
...
impl Database {
        ...
        let mut stmt = conn
            .prepare(
                "SELECT id, name, description, directory, repo_owner, repo_name, repo_branch,
                        readme_url, enabled_claude, enabled_codex, enabled_grokbuild,
                        enabled_opencode, installed_at, content_hash, updated_at
        ...
        conn.execute(
            "INSERT OR REPLACE INTO skills
             (id, name, description, directory, repo_owner, repo_name, repo_branch,
              readme_url, enabled_claude, enabled_codex, enabled_grokbuild, enabled_opencode,
    ...
    pub fn update_skill_apps(&self, id: &str, apps: &SkillApps) -> Result<bool, AppError> {
        ...
        let affected = conn
            .execute(
                "UPDATE skills SET enabled_claude = ?1, enabled_codex = ?2, enabled_grokbuild = ?3, enabled_opencode = ?4 WHERE id = ?5",
                params![apps.claude, apps.codex, apps.grokbuild, apps.opencode, id],
            )
            .map_err(|e| AppError::Database(e.to_string()))?;
        Ok(affected > 0)
    }
